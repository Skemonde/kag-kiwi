...
void onInit( CBlob@ this )
{
	...
	{
		Vehicle_addWheel( this, v, wheel_names[0], 13, 13, 0, Vec2f(-18.5f, 5.5f) + wheel_offset );
		...
		Vehicle_addWheel( this, v, wheel_names[4], 8, 8, 0, Vec2f(-10.0f, 15.0f) + wheel_offset );
	...
	CSpriteLayer@ tracks = sprite.addSpriteLayer("tracks", "tank_tracks.png", 64, 40, this.getTeamNum(), 0);
	if (tracks !is null)
	{
...
{	
	...
	if (ap !is null) {
		...
		if (driver !is null)
			for (u8 i = 1; i <= Maths::Floor(Maths::Abs(this.getVelocity().x))*2; ++i) ParticleAnimated("SmallSmoke" + (XORRandom(1)+1), this.getPosition() + Vec2f(-28*flip_factor, -6).RotateBy(this.getAngleDegrees()), Vec2f(-1*flip_factor,2).RotateBy(this.getAngleDegrees()), float(XORRandom(360)), 1.0f + XORRandom(100) * 0.01f, 2 + XORRandom(4), XORRandom(50) * -0.005f, true);
	}
}
...
bool doesCollideWithBlob( CBlob@ this, CBlob@ blob )
{
	...
	return (blob.getTeamNum() != this.getTeamNum() || blob.hasTag("vehicle") && this.getVelocity().Length() > 0.8);
}
