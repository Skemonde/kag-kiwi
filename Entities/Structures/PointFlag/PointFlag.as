	...
	if (isClient() && this.getTickSinceCreated() >= 1 && !this.hasTag("init_spritelayers"))
	{
		...
		if (flag !is null)
		{
			...
			if (anim_teamleft !is null && anim_teamright !is null)
			{
				...
				if (team < 7) flag.SetAnimation((team == 6 ? anim_teamleft : anim_teamright));
			}
		}
	}
...
void handleCapture(CBlob@ this)
{
    ...
    if (this.get_f32(capture_prop) >= (this.getTeamNum() == 255 ? capture_time/2 : capture_time))
    {
    	...
    	this.getSprite().PlaySound("vc1_capture_fanfare.ogg", 3.0f, 1.0f); //CapturePoint
    } 
