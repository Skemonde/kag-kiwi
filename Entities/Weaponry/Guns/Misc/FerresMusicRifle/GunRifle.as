void onInit(CBlob@ this)
{
	...
	string config_filename = this.get_string("gunrifle_config");
	if(config_filename == "") {
   ...
   if (cmd == this.getCommandID("shoot"))
   {
	...
	f32 speedmin = this.get_f32("gun_speedmin");
	 ...
	 for (int i = 0; i < 1; ++i) // tweak for the shotgun effect by Asu
	 {
       ...
       CBlob@ bullet = server_CreateBlobNoInit("x");
       if (bullet !is null)
       {
         float cangle = angle;
         
         Vec2f offset(30, 0); // was 15,0

         if (this.isFacingLeft())
         {
           offset.RotateBy(cangle, Vec2f(-5,-2));
...
// raycast that takes platform directions into account
bool rayCheck(Vec2f start, Vec2f end, Vec2f &out res)
{
	...
	if (getMap().getHitInfosFromRay(start, angle, diff.Length(), null, hits))
	{
...
void TakeBullet(CBlob@ this)
{
	...
	{
		//start reloading if we just ran out of ammo
		this.set_u16("gun_reload_timer", this.get_u16("gun_reload_ticks"));
		//play reload sound
		{
			CSprite@ sprite = this.getSprite();
			sprite.PlaySound(this.get_string("gun_reload_start_sound"));
		}
	}
}
