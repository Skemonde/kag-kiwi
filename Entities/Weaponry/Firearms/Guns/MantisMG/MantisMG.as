...
//Ammo
const uint8 CLIP        			= 250; //Amount of space in the clip
const uint8 TOTAL       			= 2; //The amount of 'spare' ammo a gun can store
...
//Firing
const uint8 FIRE_INTERVAL 			= 0; //Time between trigger pulls (in ticks)
...
//Multishot
const u8 BURST 						= 50; //How many rounds the gun shoots per click 
const u8 BURST_INTERVAL 			= 1; //Time between each bullet in a burst. (in ticks)
const uint8 BUL_PER_SHOT			= 1; //Pellets per bullet
const int8  B_SPREAD 				= 4; //Bullet Spread as degrees of an arc.
const bool UNIFORM_SPREAD 			= false; //If bullets are evenly spaced

//Trajectory
const Vec2f B_GRAV   				= Vec2f(0,0); //Bullet gravity drop \|/
const int8  B_SPEED  				= 2; //Bullet speed, STRONGLY AFFECTED/EFFECTS B_GRAV
const u8    B_SPEED_RANDOM 			= 0; //Up to this amount is added to the base speed randomly.
const u8    B_TTL    				= 3200; //TTL = Time To Live, bullets will live for X ticks before getting destoryed IF nothing has been hit
const u8    RICOCHET_CHANCE 		= 0; //The chance bullets will ricochet. 100 = 100% chance. Note the actual chance is lower than this amount, since due to bad code, bullets fail to ricochet about 25-50% of the time.
...
const string RELOAD_SOUND 			= "PickupGun.ogg"; //Plays when done reloading

//Bullet Sprites
const string BULLET_SPRITE 			= "dot_bullet.png";
