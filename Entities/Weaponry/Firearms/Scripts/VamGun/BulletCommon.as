...
//so we can call it with no command
void server_MakeBulletCommand(u16 holder_id, u16 gun_id, f32 angle, Vec2f pos, bool do_altfire)
{
	//if (!isServer()) return;
	...
	//print("hello from server command");
...
void HandleBulletCreation(u16 hoomanBlobId, u16 gunBlobId, f32 angle, Vec2f pos, bool do_altfire, f32[] rnd_bullet_angles, f32[] rnd_bullet_speeds)
{
	...
	for(int counter = 0; counter < b_count; ++counter) {
		...
		if (blobName == "bullet") {
			u8 bullet_team = holder.getTeamNum();
			bullet_team = gunBlob.getTeamNum();
			
			BulletObj@ bullet = BulletObj(hoomanBlobId,gunBlobId,bulletAngle,pos+holder.getVelocity(), bullet_team, holder.isFacingLeft(), vars, rnd_bullet_speeds[counter], burst_id);
