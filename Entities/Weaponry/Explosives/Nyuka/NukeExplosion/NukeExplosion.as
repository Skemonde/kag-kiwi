...
void onTick(CBlob@ this)
{
	...
	if (isClient())
	{
		if (ticks > (sound_delay * 30) && !this.hasTag("sound_played"))
		{
			...
			f32 modifier = 0.50f - (sound_delay / 3.0f);
			// print("modifier: " + modifier);
			
			if (modifier > 0.01f)
			{	
				Sound::Play("Dynamite.ogg", getDriver().getWorldPosFromScreenPos(getDriver().getScreenCenterPos()), 2.0f - (0.2f * (1 - modifier)), modifier);
			}
		}
	}
}
