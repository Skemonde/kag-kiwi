...
void onInit(CBlob@ this)
{
	...
	CSpriteLayer@ timer_units = sprite.addSpriteLayer("timer_units", "digits.png", 5, 7);
	timer_units.SetFrameIndex(6);
	if (timer_units !is null)
	{
		timer_units.SetOffset(timer_offset);
		timer_units.setRenderStyle(RenderStyle::outline);
		timer_units.SetRelativeZ(2000.0f);
		timer_units.SetVisible(false);
	}
	CSpriteLayer@ timer_tens = sprite.addSpriteLayer("timer_tens", "digits.png", 5, 7);
	timer_tens.SetFrameIndex(6);
	if (timer_tens !is null)
	{
		...
		timer_tens.SetRelativeZ(2000.0f);
		timer_tens.SetVisible(false);
	}
	CSpriteLayer@ timer_hundreds = sprite.addSpriteLayer("timer_hundreds", "digits.png", 5, 7);
	timer_hundreds.SetFrameIndex(6);
	if (timer_hundreds !is null)
	{
		...
		timer_hundreds.SetRelativeZ(2000.0f);
...
void onTick( CBlob@ this )
{
	...
	if (this.exists("death_timer") && !this.exists("death_date")) this.set_u32("death_date", getGameTime() + (this.get_u16("death_timer") * 30));
	...
	if (this.get_u32("death_date") >= getGameTime() && timer_units !is null && timer_tens !is null && timer_hundreds !is null)
	{
		...
		u32 remaining_time = (this.get_u32("death_date") + 30 - getGameTime())/30;
