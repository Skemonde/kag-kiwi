...
bool isBuildableAtPos(CBlob@ this, Vec2f p, TileType buildTile, CBlob @blob, bool &out sameTile)
{
	...
	// tilemap check
	const bool buildSolid = (map.isTileSolid(buildTile) || buildTile == CMap::tile_steel_1x1 || (blob !is null && blob.isCollidable()));
	...
	bool woodenTiles = backtile.type >= CMap::tile_wood && backtile.type <= CMap::tile_wood_d0;
	bool castleTiles = backtile.type >= CMap::tile_castle && backtile.type <= CMap::tile_castle_d0;
	...
	if ((buildTile == CMap::tile_wood && backtile.type >= CMap::tile_wood_d1 && backtile.type <= CMap::tile_wood_d0) ||
			...
			(buildTile == CMap::tile_steel_1x1 && isTileDamagedSteel(backtile.type))
			)
	{
		//repair like tiles
	}
	else if (buildTile == CMap::tile_castle && woodenTiles && !map.isInFire(p))
	{
		// can build stone on wood when not on fire, do nothing
	}
	else if (buildTile == CMap::tile_bgsteelbeam && (backtile.type == CMap::tile_bgsteelbeam || left.type == CMap::tile_bgsteelbeam || right.type == CMap::tile_bgsteelbeam))
	{
	...
	else if (buildTile == CMap::tile_castle_back && (backtile.type == CMap::tile_bgsteelbeam))
	{
		//cant build stone on steel beam background
		return false;
	}
	else if (buildTile == CMap::tile_window && (isTileWindow(backtile.type) || backtile.type == CMap::tile_castle_back || backtile.type == CMap::tile_wood_back || isTileSteelBeam(backtile.type)))
	{
		return false;
	}
	else if (buildTile == CMap::tile_steel_1x1 && (woodenTiles || castleTiles))
	{
		// can build steel block atop of stone or wood blocks
	}
	else if (backtile.type == CMap::tile_lightabsorber)
	{
		//cannot build atop of light absorber
		return false;
	}
...
u32 getCurrentBuildDelay(CBlob@ this, bool smaller_delay = false)
{
	if (smaller_delay)
		return 1;
