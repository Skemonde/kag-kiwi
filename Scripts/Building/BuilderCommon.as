...
Vec2f getBuildingOffsetPos(CBlob@ this, CMap@ map, Vec2f required_tile_space)
{
	return getBuildingOffsetPos(this.getPosition(), map, required_tile_space);
}

Vec2f getBuildingOffsetPos(Vec2f pos, CMap@ map, Vec2f required_tile_space)
{
...
CBlob@ getBuildingBlob(CBlob@ this) {
	AttachmentPoint@ tile_slot = this.getAttachments().getAttachmentPointByName("TILESLOT");
	if (tile_slot is null) return null;
	CBlob@ building_blob = tile_slot.getOccupied();
	return building_blob;
}
...
CBlob@ server_BuildBlob(CBlob@ this, BuildBlock[]@ blocks, uint index)
{
	...
	if (b.buildOnGround)
	{
		...
		Vec2f offsetPos = getBuildingOffsetPos(this.getPosition(), map, space);
	...
	if (getNet().isServer())
	{
		...
		if (blockBlob !is null)
		{
			...
			if (!b.buildOnGround)
			{
				...
				this.server_AttachTo(blockBlob, "TILESLOT");
			}
			else
			{
				shape.server_SetActive(true); // have it enable if its a shop
				blockBlob.Tag("temp blob placed");
...
void ClearCarriedBlock(CBlob@ this)
{
	...
	CBlob@ carried = getBuildingBlob(this);
