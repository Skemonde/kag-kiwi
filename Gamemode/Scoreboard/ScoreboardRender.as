...
float drawScoreboard(CPlayer@ localplayer, CPlayer@[] players, Vec2f topleft, CTeam@ team, Vec2f emblem)
{
	...
	Vec2f bottomright(Maths::Min(getScreenWidth() - 450, screenMidX+maxMenuWidth), topleft.y + (players.length + 5.5) * stepheight);
	...
	f32 kag_username_offset = 520;
	f32 ping_offset = 330;
	f32 kills_offset = 260;
	f32 deaths_offset = 190;
	//draw player table header
	GUI::DrawText("Rank", topleft, SColor(0xffffffff));
	GUI::DrawText("Nickname", Vec2f(topleft.x + nickname_offset, topleft.y), SColor(0xffffffff));
	GUI::DrawText("KAG Username", Vec2f(bottomright.x - kag_username_offset, topleft.y), SColor(0xffffffff));
	GUI::DrawText("Ping", Vec2f(bottomright.x - ping_offset, topleft.y), SColor(0xffffffff));
	GUI::DrawText("Kills", Vec2f(bottomright.x - kills_offset, topleft.y), SColor(0xffffffff));
	...
	for (u32 i = 0; i < players.length; i++)
	{
		...
		if (false)//clantag != "")
		{
			...
			GUI::DrawText(clantag, topleft + Vec2f(nickname_offset, 0), SColor(0xff888888));
			//draw name alongside
		...
		GUI::DrawText("" + username, Vec2f(bottomright.x - kag_username_offset, topleft.y), playercolour);
		...
		if (ping_in_ms < 66)
			...
			// you're dead :kag_skull:
			ping_icon_index = 4;
			
		if (playerHover && mousePos.x > bottomright.x - ping_offset + 4 && mousePos.x < bottomright.x - ping_offset + 16 + 8)
		{
			drawHoverText(formatFloat(ping_in_ms, "", 0, 0)+" ms ", Vec2f(bottomright.x - ping_offset - 16, topleft.y));
		}
...
void drawHoverText(string desc, Vec2f pos)
{
	Vec2f size(0, 0);
