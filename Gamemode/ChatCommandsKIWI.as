	...
	if (text_in.substr(0,1) == "!")
	{
		...
		if (tokens.length > 0)
		{
			...
			if (isCool || isMod)
			{
				...
				{
					f32 gvalue;
					if (tokens.length!=2) gvalue = 100;
					else gvalue = f32(parseInt(tokens[1]));
					gvalue /= 100;
				...
				else if (tokens[0] == "!cursor")
				{
					if (tokens.length > 1)
					{
						...
						CBlob@ newBlob = server_CreateBlob(name, teamNum, pos);
						bool isBuilding = newBlob.hasTag("building");
						if (isBuilding) pos = Vec2f(pos.x, pos.y - 8);
