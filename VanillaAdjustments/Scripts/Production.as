	...
	if (item !is null && hasRequirements_Tech(inv, item.requirements, missing))
	{
		...
		else // everything else
		{
			...
			if (blob !is null)
			{
				...
				{
					CBlob@ for_crate = server_CreateBlob(item.blobName, this.getTeamNum(), this.getPosition());
					blob.server_PutInInventory(for_crate);
					for_crate.server_SetQuantity(item.quantity == 65535 ? blob.maxQuantity : item.quantity);
					//SetCratePacked(blob, item.blobName, item.name, this.inventoryIconFrame);
					//blob.set_u16("msg blob", this.getNetworkID());
				}
				else
					blob.server_SetQuantity(item.quantity == 65535 ? blob.maxQuantity : item.quantity);
				
				// skemonde: i needed that quantity thing for my idea of quantity bonuses for capturing some spots on map
				// btw if out items come in crate "blob" is for crate so we don't change its quantity
				return blob;
			}
		}
	}
			...
			for (uint i = 0; i < ids.length; i++)
			{
				...
				if (blob !is null)
				{
					if (blob.getName() == "crate") {
						if (item.spawnInCrate) {
							if (blob.exists("packed") && blob.get_string("packed") == item.blobName)
								count++;
						}
					}
					...
					CInventory@ crate_inv = blob.getInventory();
					if (crate_inv !is null) {
						u16 item_count = crate_inv.getItemsCount();
						for (u16 i = 0; i < item_count; i++) {
							CBlob@ current_item = crate_inv.getItem(i);
							if (current_item !is null && current_item.getName() == item.blobName) {
								count++;
								//print("in crates" + count);
							}
						}
					}
					//print("total" + count);
