...
void updateTradeMenuGUI(CBlob@ this)
{
	const string caption = getRules().get_string("trade open menu name");
	if (caption == "") { return; }

	const int callerBlobID = getRules().get_netid("trade open menu caller");
	...
	if (!this.get("items", @items) || items is null) return;
	
	if (menu.getButtonsCount() != items.length)
	{
		warn("expected " + menu.getButtonsCount() + " buttons, got " + items.length + " items");
		return;
	}
	...
	for (uint i = 0; i < items.length; ++i)
	{
		...
		applyButtonProperties(@this, @callerBlob, @button, @item);
	}
}

void applyButtonProperties(CBlob@ shop, CBlob@ caller, CGridButton@ button, TradeItem@ t_item)
{
	...
	CBitStream missing;
	if (hasRequirements_Tech(shop.getInventory(), caller.getInventory(), t_item.reqs, missing))
	{
		tookReqs = true;
	}
	...
	string desc = t_item.getDescription();
	
	if (desc.empty()) return;
	
	SetItemDescription_Tech(button, caller, t_item.reqs, getTranslatedString(desc), takeReqsFromStorage ? storageReq.getInventory() : shop.getInventory());
}
...
void onTick(CBlob@ this)
{
	...
	if (isClient() && getRules().exists("trade open menu blob") && getRules().get_netid("trade open menu blob") == this.getNetworkID())
	{
		updateTradeMenuGUI(@this);
	}
}
...
void GetButtonsFor(CBlob@ this, CBlob@ caller)
{
	...
	if (!this.hasTag("dead"))
	{
		CButton@ button = caller.CreateGenericButton("$trade$", this.get_Vec2f("shop offset"), this, BuildTradingMenu, getTranslatedString("Shop"));
...
void BuildTradingMenu(CBlob@ this, CBlob @caller)
{
	...
	{
		getRules().set_netid("trade open menu blob", this.getNetworkID());
		getRules().set_string("trade open menu name", this.get_string("trade menu caption"));
		getRules().set_netid("trade open menu caller", caller.getNetworkID());
		...
		for (uint i = 0 ; i < items.length; i++)
		{
			...
			if (item.isSeparator)
			{
				...
				if (separator !is null)
				{
					...
					separator.SetEnabled(!item.iconName.empty());
				}
			}
			else
			{
				...
				CGridButton@ button = menu.AddButton(item.iconName, getTranslatedString(item.name), "TradingMenu.as", "Callback_Buy", Vec2f(Maths::Max(1, item.separatorIconSize.x), Maths::Max(1, item.separatorIconSize.y)), params);
				if (button !is null)
				{
