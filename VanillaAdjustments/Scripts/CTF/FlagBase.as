...
void onTick(CBlob@ this)
{
	if (getNet().isServer())
	{
		u16 flag_id = this.get_u16("flag id");
		CBlob@ flag = getBlobByNetworkID(flag_id);
		if (flag is null) {
			if ((this.get_u32("capture_moment")+300)<getGameTime()) {
				makeNewFlag(this);
			}
		}
		...
		if (!this.hasAttached())
		{
			...
			else
			{
				...
				this.Untag("nobuild sector added");
				Vec2f pos = this.getPosition();
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid)
{
	...
	else //our team
	{
		...
		if (b !is null && b.getName() == flag_name && b.getTeamNum() != this.getTeamNum())
		{
			...
			u16 base_id = b.get_u16("base_id");
			CBlob@ base = getBlobByNetworkID(base_id);
			if (base !is null)
				base.set_u32("capture_moment", getGameTime());
		}
	}
