...
#include "TreeSync.as"
const Vec2f segment_offset = Vec2f(0,-4);
...
void onInit(CBlob@ this)
{
	...
	vars.height = 0;
	vars.max_height = 5;//5 + vars.r.NextRanged(3);
	vars.grown_times = 0;
...
void GrowSprite(CSprite@ this, TreeVars@ vars)
{
	...
	for (uint i = 0; i < segments.length; i++)
	{
		...
		if (segment !is null && !segment.gotsprites)
		{
			...
			else if (segment.grown_times == 2 && segment.height > 1)
			{
				...
				if (newsegment !is null)
				{
					...
					newsegment.SetAnimation(animGrow);
					newsegment.ResetTransform();
					newsegment.SetRelativeZ(550.1f + (vars.height * 0.1f));
					newsegment.RotateBy(segment.angle, Vec2f(0, 0));
					newsegment.TranslateBy(segment.start_pos + segment_offset);
			...
			else if (segment.grown_times == 5)
			{
				...
				else if (segment.height > 1 && segment.height <= vars.max_height)  //add leaves
				{
					bool flip = false;
					...
					if (newsegment !is null)
					{
						...
						newsegment.SetAnimation(animGrow);
						newsegment.ResetTransform();
						newsegment.SetRelativeZ(-540.1f + (vars.height * 0.1f));

						newsegment.SetFacingLeft(flip);

						newsegment.SetOffset(segment.start_pos + segment_offset + Vec2f((vars.max_height - i+1) * (i > 3 ? 2.5 : 4.5), 4.0f) + (i == 1 ? Vec2f(-0.5, -2) : Vec2f_zero));
					}
					...
					if (true)
					{
						...
						if (secondnewsegment !is null)
						{
							...
							secondnewsegment.SetRelativeZ(-540.1f + (vars.height * 0.1f));

							flip = !flip;

							secondnewsegment.SetFacingLeft(flip);

							secondnewsegment.SetOffset(segment.start_pos + segment_offset + Vec2f((vars.max_height - i+1) * (i > 3 ? 2.5 : 4.5), 4.0f) + (i == 1 ? Vec2f(-0.5, -2) : Vec2f_zero));
						}
					}
				}
			}
