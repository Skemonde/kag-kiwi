...
shared class RunnerTextures
{
	...
	RunnerTextures(string _shortname, string texture_prefix, bool _gendered = true)
	{
		loaded = false;
		gendered = _gendered;
		...
		male_filename = texture_prefix+(gendered?"Male.png":"");
		female_filename = texture_prefix+(gendered?"Female.png":"");
	}
...
RunnerTextures@ fetchRunnerTexture(string shortname, string texture_prefix, bool gendered = true)
{
	RunnerTextures@ tex = null;
	string rules_key = "runner_tex_"+shortname+"_"+texture_prefix;
	if (!getRules().get(rules_key, @tex) || tex is null)
	{
		getRules().set(rules_key, RunnerTextures(shortname, texture_prefix, gendered));
		//re-fetch
...
RunnerTextures@ addRunnerTextures(CSprite@ sprite, string shortname, string texture_prefix, bool gendered = true)
{
	//fetch it or set it up
	RunnerTextures@ tex = fetchRunnerTexture(shortname, texture_prefix, gendered);
	//load it out
	tex.Load(sprite);
