	...
	for (int i = 0; i < inv.getItemsCount(); i++)
	{
		...
		if (drawn.find(name) == -1)
		{
			...
			GUI::DrawIcon(
				...
				tl + Vec2f(0, (drawn.length) * slot_size - slot_size/2),
				0.5f);
			...
			GUI::DrawIcon(
				...
				tl + Vec2f((inventory_gui_width - item_width)/2, (drawn.length) * slot_size - (item_height)/2),
				1.0f,
				this.getTeamNum());
			...
			GUI::DrawText(
				disp,
				tl + Vec2f(inventory_gui_width, (drawn.length) * slot_size - (item_height)/2),
				col);
		}
	}
}
...
void DrawCoinsOnHUD(CBlob@ this, const int coins, Vec2f tl, const int slot)
{
	if (coins > 0)
	{
		GUI::DrawIconByName("$COIN$", tl + Vec2f(0 + slot * 40, 4));
		GUI::SetFont("menu");
		GUI::DrawText("" + coins, tl + Vec2f(4 + slot * 40 , 0), color_white);
	}
}
